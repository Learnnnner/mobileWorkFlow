...
public class QueryTemplateTable {
    public static void query(RoutingContext routingContext, Vertx vertx) {
        ...
        String sql = "SELECT\n" +
                "\t`form_templates`.`id` AS `id`,\n" +
                "\t`form_templates`.`title` AS `title`,\n" +
                "\t`form_templates`.`description` AS `description`,\n" +
                "\t`form_templates`.`designer_id` AS `designer_id`,\n" +
                "\t`org_user`.`realname` AS `designer_name`\n" +
                "FROM\n" +
                "\t`form_templates`, `org_user`\n" +
                "WHERE `form_templates`.`designer_id` = `org_user`.id;";
